...
public class AutoArmor extends Module {
	...
	public static BooleanSetting elytra = new BooleanSetting("PreferElytra", false);
	public static BooleanSetting noThorns = new BooleanSetting("NoThorns", true);
	public static BooleanSetting searchCrafting = new BooleanSetting("CraftingSlots", true);
		...
		for (int a = 0; a < 4; a++) {
			if (refined[a] != -1) {
				...
				if (armor == null || armor.getItem() == Items.AIR) {
					if (!timer.hasPassAndReset((int) delay.getValue())) {
						hadtowait = true;
						continue;
					}
