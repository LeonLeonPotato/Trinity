...
public class Binding extends Component {
    public KeyBinding set;
    public Button parent;
    public int offset;
    public boolean binding = false;
    private int keyTimer = 0;

    public Binding(KeyBinding set, Button parent, int offset) {
        this.set = set;
        this.parent = parent;
        this.offset = offset;
    }
    ...
    public void render() {
        ...
        FontUtil.drawString(this.binding ? "Listening" + determineDots(keyTimer) : "Key: " + Coloring.getWHITE() + Keyboard.getKeyName(set.Char), this.parent.parent.x + 7, this.parent.parent.y + this.parent.offset + 14 + this.offset + ((14 - FontUtil.getFontHeight()) / 2f), ClickGUI.nameColorSetting.getValue().getRGB());
    }
